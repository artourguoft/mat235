...
Equations in $\mathbb{R}^{3}$ represent **surfaces** rather than **curves**. The simplest of these surfaces are planes, which are defined by a single coordinate; ex. $z=5$ is the set of all ordered triples whose $z$ coordinate is $5$.
- In general, for a constant $c$, $x=c$ is a plane parallel to the $yz$-plane, $y=c$ a plane parallel to the $xz$-plane, and $z=c$ a plane parallel to the $xy$-plane
	- Furthermore, if any of the three variables are **not explicit** in an equation, then the surface described by the equation extends infinitely along that axis; you can think of this as that variable having an implicit $0$ coefficient, allowing it to take any value
...
- Equations of the form $x^2+y^2=c$ represent **circles** with radius $\sqrt{c}$, which in $\mathbb{R}^{3}$ extend along the $z$-axis to form hollow **cylinders**
	- It follows that forms like $x^2+y^2\leq c$ represent filled-in circles and cylinders

The **distance** between two points in $\mathbb{R}^{3}$ is $\sqrt{(x_{2}-x_{1})^{2}+(y_{2}-y_{1})^{2}+(z_{2}-z_{1})^{2}}$. The equation of a **sphere** centered at $(a,b,c)$ with radius $r$ follows as $(x-a)^{2}+(y-b)^{2}+(z-c)^{2}=r^{2}$.
- Often we have equations of the form $x^{2}+y^{2}+z^{2}+ax+by+cz+d=0$; these actually represent spheres as well, and we can reveal that by grouping terms and **completing the square**:
	- Ex. $x^{2}+y^{2}+z^{2}+4x+6y+2z+6=0$ to $(x^{2}+4x+4)+(y^{2}+6y+9)+(z^{2}+2z+1)=-6+4+9+1$ to $(x+2)^{2}+(y+3)^{2}+(z+1)^{2}=8$
### <u>12.2: Vectors</u>
...
**Vector Addition:** for vectors $\mathbf{a}, \mathbf{b}$, we have $\mathbf{a}+\mathbf{b}=\mathbf{b}+\mathbf{a}$, where the sum is the vector from the initial point of one vector to the terminal point of the other
...
- This also helps us define **parallel** vectors, which are vectors that are **scalar multiples** of each other
...
Regarding magnitude, we can interpret this as the distance between two points called a **displacement** vector, where for two points $P_{1}=(x_{1},y_{1},z_{1})$ and $P_{2}=(x_{2},y_{2},z_{2})$, the displacement vector is $(x_{1}-x_{2},y_{1}-y_{2},z_{1}-z_{1})$. Furthermore, in $\mathbb{R}^{2}$ we can use the magnitude and the angle with the $x$-axis $\theta$ to determine the $x$ and $y$ components as $(\|\mathbf{a}\|\cos\theta,\|\mathbf{a\|\sin\theta})$.
...
**Dot Product:** for vectors $\mathbf{a}=(a_{1},a_{2},\dots,a_{n}),\mathbf{b}=(b_{1},b_{2},\dots,b_{n})$, we define $a\cdot b=a_{1}b_{1}+a_{2}b_{2}+\dots +a_{n}b_{n}$. Note that the dot product is a real number, not a vector.
...
To determine the **equation of a plane**, we only need a single point $P_{0}=(x_{0},y_{0},z_{0})$ on the plane and a normal vector $\mathbf{n}=a\mathbf{i}+b\mathbf{j}+c\mathbf{k}$ to the plane. Then all displacement vectors from $P_{0}$ that are perpendicular to the normal vector define the plane. 
- Take an arbitrary point $P_{1}=(x,y,z)$ in the plane, then the displacement vector $P_{0}P_{1}=(x-x_{0},y-y_{0},z-z_{0})$ must be **perpendicular to the normal** vector, so $\mathbf{n}\cdot P_{0}P_{1}=0$, from which we get the equation $a(x-x_{0})+b(y-y_{0})+c(z-z_{0})=0$
...
- If we are not given a normal vector, but are given at least $3$ non-coincident points, then we can determine two displacement vectors and use the cross product to determine a normal vector; more on that next chapter


One common physics application of the dot product is **work**, defined as $W=\mathbf{F}\cdot \mathbf{d}=\|\mathbf{F\|\|\mathbf{d\|\cos\theta}}$; the dot product of the force and distance vectors.


**Scalar Projection:** also called the **component** of $\mathbf{b}$ along $\mathbf{a}$, the scalar projection is the magnitude of the superimposed 'shadow' cast by a vector $\mathbf{b}$ onto a vector $\mathbf{a}$:
$$
\text{comp}_{\mathbf{a}}\mathbf{b}=\frac{\mathbf{a}\cdot \mathbf{b}}{\|\mathbf{a}\|}
$$
**Vector Projection:** the vector which constitutes the superimposed 'shadow' of a vector $\mathbf{b}$ onto a vector $\mathbf{a}$ is the vector projection denoted as:
$$
\text{proj}_{\mathbf{a}}\mathbf{b}=(\frac{\mathbf{a}\cdot \mathbf{b}}{\|\mathbf{a}\|})\frac{\mathbf{a}}{\|\mathbf{a}\|}=\frac{\mathbf{a}\cdot\mathbf{b}}{\|\mathbf{a}\|^{2}}
$$
Notice that the vector projection is the scalar projection times the unit vector in the direction of $\mathbf{a}$.
### <u>[13.4]: The Cross Product</u>
Recall that for any two vectors $\mathbf{a},\mathbf{b}\in \mathbb{R}^{3}$, if we trace the two ways of adding these vectors we get a parallelogram. The formula for the area of a parallelogram is $\text{base} \times \text{height}$, which in vector terms is equivalent to $\|\mathbf{a}\|(\|\mathbf{b}\|\\sin\theta)$ assuming $\mathbf{a}$ as the base. However, if we are given only the components of these two vectors, calculating the magnitudes and the angle (through dot product) can be a long process. Instead, we use the cross product.
...
- Given this definition, the **direction** of the cross product must be given by the normal vector $\mathbf{n}$ to the plane defined by $\mathbf{a},\mathbf{b}$. If we also require that $\mathbf{n}$ be a unit vector, then there are only two vectors to choose from, each pointing in a perpendicular direction to said plane. We choose by **the right-hand rule**.

Geometrically, for non-parallel vectors $\mathbf{a},\mathbf{b}\in \mathbb{R}^{3}$ with angle $0\leq\theta<\pi$ between them:
$$
\mathbf{a}\times \mathbf{b} = (\|\mathbf{a}\|\|\mathbf{b}\|\sin\theta)\mathbf{n}
$$
Note that this is the area of the parallelogram (and the magnitude of the cross product) multiplied as a **scalar** into the normal vector for the given plane. This also suggests that the cross product is **rotation invariant**; if we connect $\mathbf{a},\mathbf{b}$ at their roots and rotate the resulting object around, only the direction of the cross product (defined by $\mathbf{n}$) will change, not the magnitude.
...
$$
Note the second term is subtracted and the order of subtraction inside the brackets of that term is reversed by the definition of determinants; this is ultimately equivalent to the algebraic definition of cross products above. Also, the algebraic method spares us from having to determine a normal vector first.
...
Recall, the **equation of a plane** can be found from a single point on the plane and a normal vector. With the cross product, we only need to be given $3$ points that define the plane. Then we can determine two displacement vectors between them, and use to the cross product to determine a normal vector to the plane.
## <u>14: Partial Derivatives</u>
### <u>14.1: Functions of Several Variables</u>
A **function** $f$ **of two variables** is a rule that assigns to each order pair of real numbers $(x,y)$ in a set $D$, a unique real number denoted $f(x,y)$.
...
Functions of two variables **can exclude one of the variables**; ex. $f(x,y)=x^{2}$ is a parabola opening upward on the $z$-axis and extending infinitely in both directions along the $y$-axis. Or ex. $y=x^{2}$ which is a standard parabola extending infinitely in both directions along the $z$-axis.

**Linear functions** of two variables take the form $z=ax+by+c$ (or rearranged as $ax+by-z=-c$), and represent **planes** in $\mathbb{R}^{3}$. More on these in the next chapter.

The **level curves** of a function of two variables are curves in the $xy$-plane with equations $f(x,y)=c$ for a constant $z=c$ chosen from the range of $f$. In other words, these are sets of all points at which $f(x,y)$ is $c$ height away from the $xy$-plane. A collection of level curves for various values of $z$ is a **contour map**.
- Level curves are essentially horizontal slices of the graph of $f(x,y)$, superimposed on the $xy$-plane; substituting $z=c$ gives us the equation of the level curve in the $xy$-plane
...
**Cross-sections** are similar to level curves, but they are defined by vertical (or otherwise non-horizontal) planes, compared to level curves which are defined by strictly horizontal planes. Commonly useful cross-sections are those defined by planes where $x$ or $y$ are held constant; then we are left with $z=f(x,c)$ or $z=f(c,y)$ which are vertical slices where the curve is $z$ as function of only one of $x$ or $y$.

Functions of **three or more variables** are defined similarly, but of course difficult to visualize. Nevertheless, we can gain some insight by examining the **level surfaces** of three-variable functions, which are surfaces in $\mathbb{R}^{3}$ with equations $f(x,y,z)=c$ where $c$ is a constant. In other words, as any of the ordered triples that lie on these surfaces will have the same value of $f$. 
- These contour diagrams are **collections of level surfaces**, and we can **determine an equation** in $\mathbb{R}^{3}$ of each surface by isolating $z$ for each given $c$, and treating it as function of two variables
	- Note, this is **not always possible**, as we will see when dealing with partial derivatives
	- Conversely, we can also determine a function $g$ for which $f$ is a level surface by setting $f(x,y)-z=0=g(x,y,z)$ for $g(x,y,z)=0$
### <u>[12.4]: Linear Functions</u>
Graphs of linear functions of two variables are **planes** (as opposed to lines for functions of one variable). The defining feature of a linear function of two variables is that the **slopes** $\frac{\Delta z}{\Delta x}$ at a fixed $y$ and $\frac{\Delta z}{\Delta y}$ at a fixed $x$ are **constant**. These slopes are also the same **regardless of the fixed value**. In fact, the slope of a plane along any **same direction** is always the same constant regardless of the initial point.

These properties of linear functions are also visible when the function is presented in tabular form, where the rows and columns both have constant linear increments throughout.
...
$$
\begin{align}
f(x,y)&=z=z_0+m(x-x_0)+n(y-y_0) \\
f(x,y)&=z=c+mx+ny, \quad \text{where the $z$-intercept is $c=z_0-mx_0-ny_0=f(0,0)$}
\end{align}
$$
...
A plane can also be determined by $3$ points if we are not given any slopes, provided all $3$ points **do not lie on the same line**. If we are given points that pairwise lie parallel to the $x$ and $y$ axes, then we can simply use the difference in $z$ between those points to calculate $m,n$, and use those with any of the given points as $(x_{0},y_{0},z_{0})$ to determine our function. 
- If we are not given points that pairwise lie along axes, we use the displacement vectors to cross product to normal vector method instead

The contour diagram of a linear function will always be parallel, equally spaced straight lines. Each level curve represents the linear combinations of $(x,y)$ for that given $z$. Similarly for cross-sections; any cross sections with the same direction will yield curves with the same slope.
### <u>14.2: Limits and Continuity</u>
For a function $f$ whose domain includes points arbitrarily close to but not necessarily $(a,b)$, we say that the **limit** of $f(x,y)$ as $(x,y)$ approaches $(a,b)$ is $L$ as:
$$
\lim_{ (x,y) \to (a,b) } f(x,y) = L
$$
Recall, for single variable functions the **definition of the limit** was that the difference between $f(x)$ and $L$ can be made arbitrarily small by bounding the distance $0<|x-a|<\delta$. Now, our domain is the $xy$-plane rather than the $x$-axis. Thus, the definition changes to where the $\delta$ is a **radius** around the point $(a,b)$ on the $xy$-plane $\sqrt{(x-a)^{2}+(y-b)^{2}}$, rather than a distance away from $a$.
...
- For functions of two variables, we can approach $(a,b)$ from an infinite number of directions and in any manner (ex., all lines through $(0,0)$ in the $xy$-plane)
- However, our definition of the limit above only specified that we must be within a certain radius $\delta$, so if a limit exists then it must approach the same value regardless of where specifically within the circle of radius $\delta$ the points of approach lie
- Then, we can show that a limit does not exist by finding **at least two paths** along which the function takes on different values as we approach $(a,b)$; the simplest paths to check are the $x$-axis and $y$-axis since one value will remain constant throughout in each path
	...
	- Note, we can pick **any curve as a path**, for ex. $y=x^2$ etc., as long as it **approaches the point** in the limit

All **limit laws** apply the same as they did for single variable functions.

Another concept that holds is the **Squeeze Theorem**. This is often useful when $f$ is a quotient of polynomials; then we can bound it by terms in the numerator, and if the bounds converge to the same limit then so does $f$.

**Continuity** is defined once again by the property:
$$\lim_{(x,y) \to (a,b)} f(x,y) = f(a,b)$$We say that $f$ is **continuous on its domain** if it continuous at each point in its domain. Limits of functions that we know are continuous, like polynomials or quotients of polynomials, can again be evaluated by simply substituting in the point $(a,b)$.
...
Partial derivatives can also be defined for functions of three or more variables, but they are still defined as the change of the dependant variable with respect to a **single independent variable** with all other variables held constant.
